...
class SimpleController(Controller):
    ...
    def initialize(self, initialize_environment=True, algo_kwargs=None, run_id=None):
        ...
        if self.config.store_fields is None:
            self.store_dtype =self.evaluation_dtype
        else:
            self.store_dtype = np.dtype([(f, dt[0]) for f, dt in self.evaluation_dtype.fields.items() if f in self.config.store_fields])

        self.t = 0

        # initialize dataset
        if not self.dbase is None:
            self.dset = self.dbase.get_dset(group=self.group_id, id=self.run_id, dtype=self.store_dtype)
    ...
    def _get_dset_entry(self, evaluation):
        if self.config.store_fields is None:
            return evaluation
        else:
            return np.array(tuple([evaluation[f] for f in self.store_dtype.fields]), dtype=self.store_dtype)
    ...
    def _run_step(self):
        ...
        try:
            ...
            # record data
            if not self.dset is None:
                self.dset.add(self._get_dset_entry(evaluation))
            else:
