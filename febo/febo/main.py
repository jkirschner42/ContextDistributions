...
def initialize_framework(default_config='default.yaml', config_manager=config_manager, **kwargs):
    """
...
def create_experiment(name, overwrite=False):
    ...
    """
    config_manager.update_config(main_config)  # make sure main_config is updated
...
def load_experiment(name):
    """
    Helper function to load an experiment by name
    """

    # load config to get experiment class
    path = os.path.join(main_config.experiment_dir, name)
