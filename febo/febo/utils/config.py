...
class ConfigManager:
    ...
    def get_setting(self, section, setting):
        """
        returns the encoded value for `section.setting`
        """
        field = self.get_field(section, setting)
        return field._encode(self._data[section].get(setting, field.default))

    def get_field(self, section, setting):
        """
        returns the associated field for `section.setting`
        """
        if not section in self._fields:
            raise Exception(f"Unknown section '{section}'.")
        ...
        return self._fields[section][setting]
