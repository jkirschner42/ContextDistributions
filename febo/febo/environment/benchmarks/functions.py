...
class CamelbackTanh(BenchmarkEnvironment):
    """
    Camelback benchmark function.
    """
    ...
    def __init__(self, path=None):
        ...
        self._x0 = np.array([0.5, 0.2])
        self._x0 = np.array([-0.12977758051079197, 0.2632096107305229])
        self._max_value = np.tanh(1.03162842)
        self._domain = ContinuousDomain(np.array([-2, -1]), np.array([2, 1]))
    ...
    def f(self, x):
        ...
        yy = x[:, 1]
        y = (4. - 2.1 * xx ** 2 + (xx ** 4) / 3.) * (xx ** 2) + xx * yy + (-4. + 4 * (yy ** 2)) * (yy ** 2)
        return np.tanh(np.maximum(-y, -2.5))
...
class LinearBallConfig(BenchmarkEnvironmentConfig):
    s = ConfigField(1.)

@assign_config(LinearBallConfig)
class LinearBall(BenchmarkEnvironment):
    def __init__(self, path=None):
        super().__init__(path)
        ...
        self._domain = ContinuousDomain(-2*np.ones(self.config.dimension), 2*np.ones(self.config.dimension))

    def initialize(self):
        env_info = super().initialize()
        env_info['theta'] = self._theta
        return env_info

    def f(self, X):
        return np.inner(self._theta, X).reshape(-1,1)
